...
function marketToSlide(m: ApiMarket & {
  ...
  const tags: Record<string, string> = {
    sports: "SPORTS", crypto: "CRYPTO", politics: "POLITICS",
...
const FALLBACK_SLIDES: Slide[] = [
  {
    id: 0, tag: "AFRICA-FIRST", tagColor: "#10b981",
    headline: "Trade What Happens Next.",
    sub: "OutcomX is a prediction market for real-world events. Research, take a position, and see if your conviction pays off.",
    cta: "Explore Markets", href: "/",
    gradient: "linear-gradient(135deg, #0d1b3e 0%, #0a2a5e 50%, #0d1b3e 100%)",
    icon: <span style={{ fontSize: 64, opacity: 0.15, position: "absolute", right: 40, top: "50%", transform: "translateY(-50%)" }}>X</span>,
    accent: "#6c63ff",
  },
  {
    id: 0, tag: "LIVE NOW", tagColor: "#10b981",
    headline: "Crypto & Finance Markets",
    sub: "BTC, ETH, and macroeconomic prediction markets. 5-minute to daily durations.",
    cta: "Trade Crypto", href: "/?category=crypto",
    gradient: "linear-gradient(135deg, #0f2027 0%, #203a43 50%, #2c5364 100%)",
    icon: <Globe size={80} style={{ opacity: 0.1, position: "absolute", right: 40, top: "50%", transform: "translateY(-50%)" }} />,
    accent: "#10b981",
  },
  {
    id: 0, tag: "TRENDING", tagColor: "#6366f1",
    headline: "Sports & Esports Markets",
    sub: "Who wins the match? Take a position on football, basketball, esports and more.",
    cta: "View Sports", href: "/?category=sports",
  ...
  {
    id: 0, tag: "POLITICS", tagColor: "#ef4444",
    headline: "Political & Economic Markets",
    sub: "Elections, policy decisions, inflation — trade the outcomes that shape the world.",
    cta: "Trade Politics", href: "/?category=politics",
  ...
  {
    id: 0, tag: "GET STARTED", tagColor: "#f59e0b",
    headline: "New to Prediction Markets?",
    sub: "Register free and place your first position in under 2 minutes.",
    cta: "Sign Up Free", href: "/register",
...
export default function NewsSlideshow() {
  ...
  useEffect(() => {
    ...
    fetchSlides();
    const pollInterval = setInterval(() => { if (!cancelled) fetchSlides(true); }, 30_000);
  ...
  useEffect(() => {
    ...
    timerRef.current = setInterval(() => { setCurrent(c => (c + 1) % slides.length); setProgress(0); }, AUTO_ADVANCE_MS);
    return () => {
      if (timerRef.current) clearInterval(timerRef.current);
      if (progressRef.current) clearInterval(progressRef.current);
    };
  ...
  const hasText = !!(slide.tag || slide.headline || slide.sub);

  return (
    <div
      style={{ position: "relative", width: "100%", overflow: "hidden", borderBottom: "1px solid var(--border)" }}
    ...
    >
      ...
      >
        {/* Background image */}
        {slide.heroBanner && (
          <img src={slide.heroBanner} alt=""
            style={{ position: "absolute", inset: 0, width: "100%", height: "100%", objectFit: "cover", objectPosition: "center", pointerEvents: "none" }}
          />
        )}

        {/* Gradient overlay when text is present */}
        {hasText && (
          <div style={{
            position: "absolute", inset: 0,
            background: slide.heroBanner ? "linear-gradient(to right, rgba(0,0,0,0.72) 0%, rgba(0,0,0,0.4) 60%, rgba(0,0,0,0.05) 100%)" : "none",
        ...
        {!slide.heroBanner && (
          <>
            <div style={{ position: "absolute", right: -60, top: -60, width: 200, height: 200, borderRadius: "50%", background: `${slide.accent}15`, pointerEvents: "none" }} />
            <div style={{ position: "absolute", right: 60, bottom: -80, width: 160, height: 160, borderRadius: "50%", background: `${slide.accent}10`, pointerEvents: "none" }} />
          </>
        )}
        ...
        {hasText && (
          <div style={{ position: "relative", zIndex: 2, maxWidth: 600 }}>
            {slide.tag && (
              <span style={{ display: "inline-block", fontSize: 11, fontWeight: 700, color: slide.tagColor, marginBottom: 6, letterSpacing: "1px", textTransform: "uppercase" }}>
                {slide.tag}
              </span>
            )}
            {slide.headline && (
              <h2 className="slideshow-headline" style={{ fontSize: "clamp(15px, 3vw, 30px)", fontWeight: 800, color: "#ffffff", margin: "0 0 6px", lineHeight: 1.25, letterSpacing: "-0.3px" }}>
                {slide.headline}
              </h2>
            )}
            {slide.sub && (
              <p className="slideshow-sub" style={{ fontSize: 13, color: "rgba(255,255,255,0.72)", margin: "0 0 12px", lineHeight: 1.45, maxWidth: 440 }}>
                {slide.sub}
              </p>
            )}
            {slide.cta && (
        ...
        {/* Pure image slide — CTA only if set */}
        {!hasText && slide.cta && (
        ...
        {/* Nav arrows */}
        <button onClick={e => { e.stopPropagation(); prev(); }} style={{ position: "absolute", left: 12, top: "50%", transform: "translateY(-50%)", width: 32, height: 32, borderRadius: "50%", background: "rgba(255,255,255,0.15)", border: "1px solid rgba(255,255,255,0.2)", color: "#fff", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 5, transition: "background 0.2s" }} onMouseEnter={e => (e.currentTarget.style.background = "rgba(255,255,255,0.25)")} onMouseLeave={e => (e.currentTarget.style.background = "rgba(255,255,255,0.15)")}>
          <ChevronLeft size={16} />
        </button>
        <button onClick={e => { e.stopPropagation(); next(); }} style={{ position: "absolute", right: 44, top: "50%", transform: "translateY(-50%)", width: 32, height: 32, borderRadius: "50%", background: "rgba(255,255,255,0.15)", border: "1px solid rgba(255,255,255,0.2)", color: "#fff", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 5, transition: "background 0.2s" }} onMouseEnter={e => (e.currentTarget.style.background = "rgba(255,255,255,0.25)")} onMouseLeave={e => (e.currentTarget.style.background = "rgba(255,255,255,0.15)")}>
          <ChevronRight size={16} />
        </button>

        {/* Close */}
        <button onClick={e => { e.stopPropagation(); setVisible(false); }} style={{ position: "absolute", right: 10, top: 10, width: 28, height: 28, borderRadius: "50%", background: "rgba(0,0,0,0.3)", border: "1px solid rgba(255,255,255,0.15)", color: "rgba(255,255,255,0.7)", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 10, transition: "all 0.2s" }} onMouseEnter={e => { (e.currentTarget.style.background = "rgba(0,0,0,0.5)"); (e.currentTarget.style.color = "#fff"); }} onMouseLeave={e => { (e.currentTarget.style.background = "rgba(0,0,0,0.3)"); (e.currentTarget.style.color = "rgba(255,255,255,0.7)"); }}>
          <X size={13} />
        </button>
      </div>
      ...
      <div style={{ position: "absolute", bottom: 10, left: "50%", transform: "translateX(-50%)", display: "flex", gap: 6, zIndex: 10 }}>
        {slides.map((_, i) => (
          <button key={i} onClick={e => { e.stopPropagation(); goTo(i); }} style={{ width: i === current ? 20 : 6, height: 6, borderRadius: 3, background: i === current ? slide.accent : "rgba(255,255,255,0.3)", border: "none", cursor: "pointer", padding: 0, transition: "all 0.3s ease" }} />
        ))}
      </div>
